...
class WebBackendClient:
    ...
    def __init__(self, http_client: httpx.AsyncClient, tg_id: Optional[int], bot_secret: str):
    ...
    async def renew_key(self, email: str, tariff_id: int, months: int) -> dict:
        """POST /api/v1/keys/{email}/renew - Renew a VPN key."""
        resp = await self._client.post(
            f"/api/v1/keys/{email}/renew",
            headers=self._get_headers(),
            json={"tariff_id": tariff_id, "months": months},
        )
        resp.raise_for_status()
        return resp.json()
    ...
    async def create_renewal_payment(self, email: str, tariff_id: int, months: int) -> dict:
        ...
        resp = await self._client.post(
            "/api/v1/payments/create",
            headers=self._get_headers(),
            json={"email": email, "tariff_id": tariff_id, "months": months},
        )
        resp.raise_for_status()
        return resp.json()
