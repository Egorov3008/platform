# Документация по модулю database ⚠️ Legacy

> ⚠️ **ВАЖНО:** Бот **больше не обращается к базе данных напрямую**. Все данные приходят из backend API через `BackendAPIClient` (`api/backend_client.py`).
> Модуль `database/` (asyncpg, `BaseRepository`, `DataService`) остался в кодовой базе как legacy, но фактически используется только **backend'ом**.
> Бот использует `CacheService` для локального кэширования данных, полученных из backend.

Модуль `database` предоставлял абстракцию для работы с базой данных в проекте Bot_3xui_vpn. Он реализует слой доступа к данным с использованием паттерна Repository и библиотеки asyncpg для асинхронного взаимодействия с PostgreSQL.
