# Модуль синхронизации (services/synchron/) ❌ Переехал в backend

> ⚠️ **ВАЖНО:** Весь модуль `services/synchron/` удалён из бота и переехал в `backend/services/synchron/`.
> Фоновая синхронизация теперь запускается через `backend/background/scheduler.py` (APScheduler, каждые 3 часа).

Модуль отвечал за синхронизацию данных между **XUI-панелью**, **кешем** и **базой данных**. Запускался каждые 3 часа как фоновая задача и был доступен для ручного запуска из админ-панели.
