...
AdminStatsGetter.get_data() calculates:
- Total users
...
def make_key(
    ...
    """Build a Key with expiry_time relative to now.
...
@pytest.fixture
def mock_model_data():
    """Mock ServiceDataModel with users and keys."""
...
class TestAdminStatsGetterResultStructure:
    ...
    async def test_stores_all_keys_in_dialog_data(
        ...
        """get_data() must store all_keys in dialog_data for handlers."""
        keys = [make_key("a@b.com", 111, 10 * 24 * 3600 * 1000)]
        mock_model_data.users.get_all.return_value = []
        ...
        assert mock_dialog_manager.dialog_data["all_keys"] == keys

    async def test_exception_returns_error_message(
        self, mock_model_data, mock_dialog_manager
    ):
        """When users.get_all() raises, get_data() returns error dict (no crash)."""
        mock_model_data.users.get_all.side_effect = RuntimeError("DB offline")
...
class TestAdminStatsGetterTotalUsers:
    ...
    async def test_zero_users(self, mock_model_data, mock_dialog_manager):
...
class TestAdminStatsGetterRegistrations:
    ...
    async def test_new_user_this_week(
        self, mock_model_data, mock_dialog_manager
    ):
        """User registered this week should count in reg_week."""
        now = datetime.now(timezone.utc)
        users = [make_user(1, created_at=now - timedelta(days=1))]
        mock_model_data.users.get_all.return_value = users
        mock_model_data.keys.get_all.return_value = []
    ...
    async def test_old_user_not_counted_this_week(
        ...
        users = [make_user(1, created_at=monday)]
        mock_model_data.users.get_all.return_value = users
        mock_model_data.keys.get_all.return_value = []
    ...
    async def test_registrations_month(
        self, mock_model_data, mock_dialog_manager
    ):
        """User registered this month should count in reg_month."""
        now = datetime.now(timezone.utc)
        users = [make_user(1, created_at=now - timedelta(days=5))]
        mock_model_data.users.get_all.return_value = users
        mock_model_data.keys.get_all.return_value = []
    ...
    async def test_registrations_year(
        self, mock_model_data, mock_dialog_manager
    ):
        """User registered this year should count in reg_year."""
        now = datetime.now(timezone.utc)
        users = [make_user(1, created_at=now - timedelta(days=30))]
        mock_model_data.users.get_all.return_value = users
        mock_model_data.keys.get_all.return_value = []
...
class TestAdminStatsGetterChurn:
    ...
    async def test_churned_user_this_week(
        self, mock_model_data, mock_dialog_manager
    ):
        ...
        # Ключ истёк 2 дня назад
        expiry_ms = int((now - timedelta(days=2)).timestamp() * 1000)

        user = make_user(1, created_at=monday - timedelta(days=30))
        key = Key(
            email="churned@b.com",
            tg_id=1,
            client_id="c1",
        ...
        mock_model_data.users.get_all.return_value = [user]
        mock_model_data.keys.get_all.return_value = [key]
    ...
    async def test_active_user_not_churned(
        self, mock_model_data, mock_dialog_manager
    ):
        ...
        user = make_user(1, created_at=monday - timedelta(days=30))
        key = make_key("active@b.com", 1, 10 * 24 * 3600 * 1000)  # expires in 10 days
    ...
    async def test_user_without_keys_not_churned(
        ...
        now = datetime.now(timezone.utc)
        user = make_user(1, created_at=now - timedelta(days=30))
    ...
    async def test_churn_month(
        self, mock_model_data, mock_dialog_manager
    ):
        """User whose key expired this month should count in churn_month."""
        now = datetime.now(timezone.utc)
        user = make_user(1, created_at=now - timedelta(days=60))
        key = make_key("churned@b.com", 1, -5 * 24 * 3600 * 1000)  # 5 days ago
    ...
    async def test_churn_year(
        self, mock_model_data, mock_dialog_manager
    ):
        """User whose key expired this year should count in churn_year."""
        now = datetime.now(timezone.utc)
        user = make_user(1, created_at=now - timedelta(days=400))
        key = make_key("churned@b.com", 1, -30 * 24 * 3600 * 1000)  # 30 days ago
...
class TestAdminStatsGetterBlocked:
    ...
    async def test_counts_blocked(
        self, mock_model_data, mock_dialog_manager
    ):
        """Blocked users must be counted correctly."""
        users = [
            make_user(1),
            make_user(2, is_blocked=True),
            make_user(3, is_blocked=True),
        ]
        mock_model_data.users.get_all.return_value = users
        mock_model_data.keys.get_all.return_value = []
