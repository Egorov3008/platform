...
Цель — **полная перестройка** пользовательского кабинета в **одностраничный профиль** как единый продукт. Стек прежний (vanilla-JS SPA + FastAPI-прокси). **Визуальный язык — стиль лендинг-страницы** (`web/landing/style.css`, тема «Premium Dark»): тёмно-синий фон `#020617→#0b1121` со sky-blue радиальными бликами + сетка 64px, акцент `#0ea5e9`, glassmorphism-карточки (`rgba(30,41,59,0.65)` + `backdrop-filter: blur(12px)`), Inter + mono для ключей, градиентные кнопки с glow. CSS-переменные и базовые стили переиспользуются из лендинга (общие `:root`-переменные, `.btn-primary`/`.btn-secondary`, pill-бейджи, карточки). Логотип — `dlya.svoih` (точка акцентным цветом). **Mobile-first, responsive:** брейкпоинт `@media (min-width:640px)` перестраивает тарифы из стопки в 3 колонки; детали ключа построчно; тач-цели ≥40px, шрифт ≥12px. Контракт не меняется: web не лезет в БД, только через `WebBackendClient` с `X-Bot-Secret`.
...
- Отдельный редизайн лендинга — кабинет лишь переиспользует его визуальный язык; сам `web/landing/` не трогаем.
...
### Responsive / мобильное поведение

Mobile-first, один лейаут адаптируется через `@media (min-width:640px)`:
- **Topbar** — на мобильном компактный: логотип + бейдж + короткое имя; «Выйти» → иконка/компактная кнопка.
- **Профиль-шапка** — на мобильном имя/дата сверху, бейджи в строку с wrap, сводка отдельной строкой.
- **Аккордеон ключей** — тач-цель во всю ширину; развёрнутые детали построчно («метка : значение»); кнопки «Скопировать/Продлить» в ряд `flex:1`, «Удалить» на всю ширину отдельной кнопкой.
- **Тарифы** — стопка карточек на мобильном (1 колонка), 3 колонки на ≥640px.
- Тач-цели ≥40px, шрифт ≥12px, отступы по краям 12–16px. Без сборки — чистый CSS `@media`.
